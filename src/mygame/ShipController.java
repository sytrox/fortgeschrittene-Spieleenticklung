...
public class ShipController extends AbstractControl {
    ...
    protected void controlUpdate(float tpf) {
        ...
        diffVectorBuff = this.spatial.getLocalTranslation().subtract(newPosition).divide(3);
        diffVectorBuff.z = 3;
        ...
        if (!(moveVector.length() < 3.18)) {
            ...
            if (!arrived) {
                arrived = true;
                tpfCounterX = FastMath.asin(diffVectorBuff.x);
                tpfCounterY = FastMath.acos(diffVectorBuff.y);
                Log.debug("nicht schon iweder" + tpfCounterX);
            }
            this.spatial.setLocalTranslation(newPosition.x + FastMath.sin(tpfCounterX) * 3, newPosition.y + FastMath.cos(tpfCounterY) * 3, 3.0f);
            this.spatial.lookAt(new Vector3f(newPosition.x + FastMath.sin(tpfCounterX + 0.001f) * 3, newPosition.y + FastMath.cos(tpfCounterY + 0.001f) * 3, 3.0f), Vector3f.UNIT_Z);
        }
