...
public class LayoutParser {
	...
	try {
	    // Get Cursors
			/*
	     * InputStream file = Screen.class.getClassLoader().getResourceAsStream( filePath ); DocumentBuilderFactory dbf =
	     * DocumentBuilderFactory.newInstance(); DocumentBuilder db = dbf.newDocumentBuilder(); Document doc = db.parse(file);
	...
	for (int s = 0; s < nodeLst.getLength(); s++) {
		...
		if (fstNode.getNodeName().equals("component")) {
		    ...
		    if (el == null) {
			((GameState) state).addScreenElement(element);
		    } else {
			el.addChild(element);
		    }
		}
    ...
    private Element createElement(Element el, Class c, String id, String pos, String dim, String rsb, String img, String ori, String flag,
	...
	if (className.equals("TabControl") || className.equals("LoginBox") || className.equals("DialogBox") || className.equals("AlertBox")
		|| className.equals("ChatBox") || className.equals("ChatBoxExt") || className.equals("OSRViewPort")) {
	    styleName = "Window";
	}
	...
	if (className.equals("Label")) {
	    ...
	    return new SelectList(screen, id, position, dimensions) {
		@Override
